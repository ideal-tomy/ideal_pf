...
export const DEMOS = [
  {
    ...
    linkState:'available',
    experienceNote:'用意された写真と下書きで試せます。サンプルはAPIキー不要。提出・通知はデモ内の体験です。',
    when:'写真整理や報告書づくり、提出後の確認に時間がかかる現場に。写真の分類・命名から下書きの確認、管理側への引き継ぎまでを見直すヒントをご紹介します。',
  ...
  },
  {
    ...
    plain:'希望からシフト案をつくる',
    lead:'希望提出→最適案作成→確定通知→急な欠勤対応までの代表3手を、店舗業務画面で体験できます。',
    one:'飲食店のシフトを、希望提出から急な欠勤の穴埋めまで3手で体験できます。',
    audience:'店長と店舗運営責任者',
    url:'https://shift-demo-ten.vercel.app/',
    linkState:'available',
    experienceNote:'ルールによる割当デモです（数字はAIに作らせない安心設計）。',
    when:'紙や表計算でシフトを組み、急な欠勤のたびに連絡に追われている店舗を想定しています。',
    can:['希望提出から最適案作成・確定通知までの代表フローを体験できます','急な欠勤連絡からの代替スタッフ選定・穴埋めを追えます','スマホ片手で全ロールの画面操作を確認できます'],
    planned:[],
    shots:[
